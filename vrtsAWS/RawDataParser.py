...
# Store infomration about entering and exiting car path or road
...
class RawDataParser(BaseDataParser):
    ...
    def _get_num_road_entrances(self, trial_dict):
        ...
        if VariableNames.HeadAngleOnEnterRoad not in trial_dict:
            return 0
        # check that there are HeadAngles for the RFC data, if not, no road entrances/practice trials.
        if not trial_dict[VariableNames.HeadAngleOnEnterRoad]:
            num_spss_lines = 0
        else:
            # one line for each road entrance
            num_spss_lines = len(
                trial_dict[VariableNames.HeadAngleOnEnterRoad])
            ...
            for var_name in var_list:
                if var_name == VariableNames.RoadEntrance:
                    ...
                    if trial_dict.get(var_name)[0] != None:
                        # SimHit returns a Tuple of lists... hnnng.
        ...
        for var_name in var_list:
            ...
            if value is not NO_VALUE:
                if var_name in tuple_variables:
                    # TODO: HACK for pilot data use the RFC data
                    value = value[0]
    ...
    def get_spss_output(self):
        ...
        for participant_trial in self.__trialsDataDictionary:
            ...
            road_entrance_lines = self._get_road_entrance_lines(
                participant_trial, trial_dict)
            spss_lines.extend(road_entrance_lines)

            summary_line = self._get_summary_line(
                participant_trial, trial_dict)
    ...
    def parseTrials(self):
        ...
        for participant_trial in trials:
            ...
            # self.__participantId
            trialDataDictionary[VariableNames.ParticipantId] = participant_session.get_participant_id(
            )
            ...
            trialDataDictionary[VariableNames.TrialType] = TrialType.toStr(
                participant_trial.get_trial_type())
            ...
            trialDataDictionary[VariableNames.Phase] = Phase.toStr(
                self._participant_session.get_phase())
#
#			# TRIAL RESULT
...
#
#			#DURATION
            trialDataDictionary[VariableNames.TrialDuration] = self.calculateTrialDuration(
                trialData)
            trialDataDictionary[VariableNames.DurationInRoad] = self.calculateDurationInRoad(
                trialData)  # for each road entrance [dur1, dur2, etc]
...
#			self.__logger.info("Traffic volume per minute: " + str(trialDataDictionary[VariableNames.TrafficVolumePerMinute]))
    ...
    def calculateJumpBacks(self, trialData):
        ...
        first_car_arrived_t = trialData.get_time_first_car_arrived(
            Direction.RIGHT)
        ...
        for i in range(len(positionData)-1):
            ...
            if (inRoad):
                ...
                velocity_z = super(RawDataParser, self).calculateParticipantMomentaryVelocityZ(
                ...
                if trialData.get_closest_car(t, Direction.RIGHT) == None:
                    ...
                    self.__logger.info(
                        "Warning: (calculateJumpBacks) could not find closest car - stopping JumpBack calculation")
                    return NO_VALUE_NUM
    ...
    def calculateSimulatedHitHMDV(self, trialData):
        rightFacingSimulatedHits = self.__calculate_simulated_hits_HMDV(
            trialData, Direction.RIGHT)  # a list
        leftFacingSimulatedHits = self.__calculate_simulated_hits_HMDV(
    ...
    def calculateSimulatedHitRWV(self, trialData):
        rightFacingSimulatedHits = self.__calculate_simulated_hits_RWV(
            trialData, Direction.RIGHT)
        leftFacingSimulatedHits = self.__calculate_simulated_hits_RWV(
            trialData, Direction.LEFT)
    ...
    def calculateHeadAngleOnEnterRoad(self, trialData):
        head_angles = self.__calculate_head_angle_on_enter_road(
            trialData)  # head angle on each enter road
        return head_angles

    def calculateMeanHeadAngleInRoad(self, trialData):
        # mean head angle each time the participant is in the road
        head_angles = self.__calculate_mean_head_angle_in_road(trialData)
        return head_angles

    def calculatePercentCheckingInRoad(self, trialData):
        #		 percent_looking = [[ON_SIDEWALK, IN_ROAD],...]
        # % time that a participant is more than the partial check value in a road/sidewalk
        percent_looking = self.__calculate_percent_time_looking(trialData)
        percent_checking_road = [x[PercentChecking.IN_ROAD]
                                 for x in percent_looking]
        return percent_checking_road

    def calculatePercentCheckingOnSidewalk(self, trialData):
        # % time that a participant is more than the partial check value in a road/sidewalk
        percent_looking = self.__calculate_percent_time_looking(trialData)
        percent_checking_sidewalk = [
            x[PercentChecking.ON_SIDEWALK] for x in percent_looking]
    ...
    def calculateMeanHeadAngleInCarPath(self, trialData):
        ...
        mean_head_angle = self._calculate_mean_head_angle_in_car_path(
            trialData)
    ...
    def calculatePercentCheckingInCarPath(self, trialData):
        ...
        percent_checking = self._calculate_percent_checking_in_car_path(
            trialData)
    ...
    def calculateDurationInCarPath(self, trialData):
        ...
        car_path_detail = trialData.get_last_enter_car_path_detail(
    ...
    def __calculate_head_angle_on_enter_road(self, trialData):
        ...
        for road_detail in road_details:
            yaw = trialData.get_participant().get_moment(
                road_detail.get_enter_time()).get_yaw()
    ...
    def _calculate_head_angle_on_enter_car_path(self, trialData):
        ...
        yaw = trialData.get_participant().get_moment(
            car_path_detail.get_enter_time()).get_yaw()
        return yaw
    ...
    def __calculate_mean_head_angle_in_road(self, trialData):
        ...
        for road_detail in road_details:
            ...
            in_road_times = [t for t in sorted(
                participant.get_participant_dict()) if t >= enter_time and t < exit_time]
    ...
    def _calculate_mean_head_angle_in_car_path(self, trialData):
        ...
        if not exit_time:
            # Operator intervened or hit
            exit_time = participant.get_last_moment().get_time()
        in_car_path_times = [t for t in sorted(
            participant.get_participant_dict()) if t >= enter_time and t < exit_time]
        ...
        if num_keys < 1:
            ...
            self.__logger.info(
                "Warning: (mean_head_angle_in_car_path) no time in car path - cannot calculate")
            return NO_VALUE_NUM
    ...
    def _calculate_percent_checking_in_car_path(self, trialData):
        ...
        in_car_path_times = [t for t in sorted(participant.get_participant_dict(
        ).iterkeys()) if t >= enter_time and t < exit_time]
        num_keys = len(in_car_path_times)
        if num_keys < 1:
            print "Warning: (percent_checking_in_car_path) no time in car path - cannot calculate"
            self.__logger.info(
                "Warning: (percent_checking_in_car_path) no time in car path - cannot calculate")
            return NO_VALUE_NUM

        for t in in_car_path_times:
            checking_state_counter[participant.get_moment(
                t).get_checking_state()] += 1
            # print "IN CAR PATH: t: %s, state %s" % (t, CheckingState.toStr(participant.get_moment(t).get_checking_state()))

        percent_full_checking = float(
            checking_state_counter[CheckingState.FULL_CHECKING_LEFT]) / num_keys
    ...
    def __calculate_simulated_hits_HMDV(self, trialData, direction):
        ...
        if meanVel > (NO_VALUE_NUM-1):
            self.__logger.info(
                "No practice trials mean walking speed: can't calculate SimulatedHit.")
            return None
        ...
        for road_detail in road_details:
            ...
            sim_hits.append(self._get_sim_hit(
                trialData, direction, meanVel, moment))
    ...
    def __calculate_simulated_hits_RWV(self, trialData, direction):
        ...
        if meanVel > (NO_VALUE_NUM-1):
            self.__logger.info(
                "No speed trials mean walking speed: can't calculate SimulatedHit.")
            return None
    ...
    def _get_sim_hit(self, trialData, direction, meanVel, moment):
        ...
        for trial_car in cars:
            ...
            front_bumper_distance_x = trial_car.get_front_bumper_distance_from_x(
                t, x)
            rear_bumper_distance_x = trial_car.get_rear_bumper_distance_from_x(
                t, x)
    ...
    def calculateVelocityOnEnterRoad(self, trialData):
        ...
        path_detail = trialData.get_last_enter_road_detail(
            ParticipantDirection.FORWARD)
    ...
    def calculateMeanVelocityInCarPath(self, trialData):
        ...
        path_detail = trialData.get_last_enter_car_path_detail(
            ParticipantDirection.FORWARD)
        if not path_detail:
            self.__logger.info(
                "MeanVelocityInCarPath: Can't calculate, never entered a car's path.")
    ...
    def calculateVelocityOnEnterCarPath(self, trialData):
        ...
        '''
        velOnEnterRightCarPath = self.__calculate_velocity_on_enter_car_path(
            trialData, Direction.RIGHT)
        velOnEnterLeftCarPath = self.__calculate_velocity_on_enter_car_path(
            trialData, Direction.LEFT)
    ...
    def calculateVelocityOnExitCarPath(self, trialData):
        ...
        velOnExitRightCarPath = self.__calculate_velocity_on_exit_car_path(
            trialData, Direction.RIGHT)
        velOnExitLeftCarPath = self.__calculate_velocity_on_exit_car_path(
            trialData, Direction.LEFT)
    ...
    def __calculate_velocity_on_exit_car_path(self, trialData, direction):
        ...
        path_detail = trialData.get_first_exit_car_path_detail(
            ParticipantDirection.FORWARD)
    ...
    def calculateMaximumVelocityInCarPath(self, trialData):
        ...
        '''
        maxVelocityInRightCarPath = self.__calculate_maximum_velocity_in_car_path(
            trialData, Direction.RIGHT)
        maxVelocityInLeftCarPath = self.__calculate_maximum_velocity_in_car_path(
            trialData, Direction.LEFT)
    ...
    def __calculate_maximum_velocity_in_car_path(self, trialData, direction):
        ...
        last_enter_car_path_detail = trialData.get_last_enter_car_path_detail(
            ParticipantDirection.FORWARD)
    ...
    def calculateMinimumVelocityInCarPath(self, trialData):
        ...
        interested in the last time they're in the car's path.
        '''
        minVelocityInRightCarPath = self.__calculate_minimum_velocity_in_car_path(
            trialData, Direction.RIGHT)
        minVelocityInLeftCarPath = self.__calculate_minimum_velocity_in_car_path(
            trialData, Direction.LEFT)
    ...
    def calculate_percent_closest_LFC_in_view_in_near_lane(self, trial_data):
        ...
        # Direction refers to the direction cars are facing.
        right_checks = self._get_car_in_view_checks_in_near_lane(
            trial_data, Direction.LEFT)
    ...
    def calculate_percent_closest_RFC_in_view_in_near_lane(self, trial_data):
        ...
        left_checks = self._get_car_in_view_checks_in_near_lane(
            trial_data, Direction.RIGHT)
    ...
    def calculate_percent_time_closest_LFC_in_view_in_far_lane(self, trial_data):
        # Users check right for left facing cars.
        right_checks = self._get_car_in_view_checks_in_far_lane(
            trial_data, Direction.LEFT)
        if right_checks is None:
            return [NO_VALUE_NUM]
    ...
    def calculate_percent_time_closest_RFC_in_view_in_far_lane(self, trial_data):
        left_checks = self._get_car_in_view_checks_in_far_lane(
            trial_data, Direction.RIGHT)
        if left_checks is None:
            return [NO_VALUE_NUM]
    ...
    def calculate_percent_time_closest_LFC_in_view_between_car_paths(self, trial_data):
        right_checks = self._get_car_in_view_checks_between_car_paths(
            trial_data, Direction.LEFT)
        if right_checks is None:
            return [NO_VALUE_NUM]
    ...
    def calculate_percent_time_closest_RFC_in_view_between_car_paths(self, trial_data):
        left_checks = self._get_car_in_view_checks_between_car_paths(
            trial_data, Direction.RIGHT)
        if left_checks is None:
            return [NO_VALUE_NUM]
    ...
    def calculate_is_looking_at_far_lane_car_on_middle_of_road_entry(self, trial_data):
        ...
        if not all(right_checks):
            return [NO_VALUE_NUM]
        last_entry_between_car_path_detail = trial_data.get_last_enter_between_car_path_detail(
            ParticipantDirection.FORWARD)
...
####
    ...
    def _calculate_percent_checking_in_road(self, trial_data, checks_in_road):
        ...
        result = []
        # not sure that this distinction matters
        road_entries = trial_data.get_road_details_forward_entrance()
        if not road_entries:
            return [NO_VALUE_NUM]  # None
    ...
    def calculate_percent_closest_car_in_view_between_curb_and_car_path(self, trial_data):
        checks = self._get_car_in_view_checks_between_curb_and_car_path(
            trial_data)
        if checks is None:
            return [NO_VALUE_NUM]
        return self._calculate_percent_checking_between_curb_and_car_path(trial_data, checks)
    ...
    def _calculate_pct_of_checks_with_car_in_view(self, trial_data, checks):
        ...
        for entry_number, road_entry in enumerate(road_entries):
            ...
            if not checks_this_entry:
                # they never checked (or never entered car path)
                result.append(NO_VALUE_NUM)
                continue
            total_check_time = 0
            ...
            for check_start, check_end in checks_this_entry:
                ...
                t = check_start
                while t <= check_end:
                    closest_car = trial_data.get_closest_car(
                        t, Direction.RIGHT)
                    car_in_view = self._is_car_visible(
                        closest_car, participant, t)
    ...
    def _get_checks_on_sidewalk(self, trial_data, car_in_view=False, check_angle=None, max_check_angle=None):
        ...
        # a list of checks for each time the participant was on the sidewalk
        checks_per_entry = []
        ...
        for road_entry in road_entries:
            ...
            if car_in_view is True:
                checks = self._get_times_with_closest_car_in_view(
                    trial_data, start_index, end_index, Direction.LEFT)
            # otherwise, we're using check_angle (and possibly max_check_angle) to find checks
            else:
                checks = self._get_checks_at_angle(
                    trial_data, start_index, end_index, check_angle, max_check_angle)
    ...
    def _get_car_in_view_left_checks_in_road(self, trial_data):
        # Direction refers to the direction cars are facing, not the direction of the check.
        left_checks = self._get_checks_in_road(
            trial_data, Direction.RIGHT, car_in_view=True)

        return left_checks

    def _get_car_in_view_right_checks_in_road(self, trial_data):
        # Direction refers to the direction cars are facing, not the direction of the check.
        right_checks = self._get_checks_in_road(
            trial_data, Direction.LEFT, car_in_view=True)

        return right_checks

    def _get_car_in_view_checks_in_near_lane(self, trial_data, direction):
        # Direction refers to the direction a car is facing, NOT the direction of a check.
        if direction == Direction.RIGHT:
            checks = self._get_checks_in_near_lane(
                trial_data, Direction.RIGHT, car_in_view=True)
    ...
    def _get_car_in_view_checks_in_far_lane(self, trial_data, direction):
        # Direction refers to the direction a car is facing, NOT the direction of a check.
        if direction == Direction.RIGHT:
            checks = self._get_checks_in_far_lane(
                trial_data, Direction.RIGHT, car_in_view=True)
        elif direction == Direction.LEFT:
            checks = self._get_checks_in_far_lane(
                trial_data, Direction.LEFT, car_in_view=True)
    ...
    def _get_car_in_view_checks_between_car_paths(self, trial_data, direction):
        if direction == Direction.RIGHT:
            checks = self._get_checks_between_car_paths(
                trial_data, Direction.RIGHT, car_in_view=True)
        elif direction == Direction.LEFT:
            checks = self._get_checks_between_car_paths(
                trial_data, Direction.LEFT, car_in_view=True)
    ...
    def _get_checks_in_road(self, trial_data, direction, car_in_view=False, check_angle=None, max_check_angle=None):
        ...
        for road_entry in road_entries:
            ...
            if car_in_view is True:
                checks = self._get_times_with_closest_car_in_view(
                    trial_data, start_index, end_index, direction)
            # otherwise, we're using check_angle (and possibly max_check_angle) to find checks
            else:
                checks = self._get_checks_at_angle(
                    trial_data, start_index, end_index, check_angle, max_check_angle)
        ...
        for road_entry in road_entries:
            ...
            if car_in_view is True:
                if direction == Direction.LEFT:
                    checks = self._get_times_with_closest_left_facing_car_in_view(
                        trial_data, start_index, end_index, direction)
                elif direction == Direction.RIGHT:
                    checks = self._get_times_with_closest_right_facing_car_in_view(
                        trial_data, start_index, end_index, direction)
    ...
    def _get_checks_in_far_lane(self, trial_data, direction, car_in_view=False):
        ...
        for road_entry in road_entries:
            ...
            if car_in_view is True:
                ...
                # Don't care about checks without on-coming traffic. This is only used with 'car_in_view' logic.
                checks = NO_VALUE_NUM
            checks_per_entry.append(checks)
    ...
    def _get_checks_at_angle(self, trial_data, start_index, end_index, check_angle, max_check_angle):
        ...
        participant = trial_data.get_participant()
        orientation_data = participant.get_orientation_data()[
    ...
    def _get_times_with_closest_right_facing_car_in_view(self, trial_data, start_index, end_index, direction):
        ...
        for data_point in orientation_data:
            ...
            t = data_point[OrientationData.TIME]
            closest_car = trial_data.get_closest_car(t, direction)
            car_visible = self._is_right_facing_car_visible(
                closest_car, participant, t)
    ...
    def _get_times_with_closest_left_facing_car_in_view(self, trial_data, start_index, end_index, direction):
        ...
        for data_point in orientation_data:
            ...
            t = data_point[OrientationData.TIME]
            closest_car = trial_data.get_closest_car_to_participant_at_time(
                t, direction)
            if closest_car == None:
                car_visible = False
            else:
                car_visible = self._is_left_facing_car_visible(
                    closest_car, participant, t)
    ...
    def calculateHits(self, trialData):
        ...
        rightFacingHits = self.__get_participant_hits(
            trialData, Direction.RIGHT)
    ...
    def __calculate_near_misses(self, trialData, direction):
        ...
        for car_path_detail in car_path_exits:
            ...
            trial_car = trialData.get_closest_car(
                exit_car_path_time, direction)
            if trial_car is None:
                continue

            participant_x = trialData.get_participant().get_x_position(exit_car_path_time)
            next_car_arrival_time = trial_car.get_time_from_x(
                exit_car_path_time, participant_x)
            if next_car_arrival_time < 1:
                # print "NEAR MISS at t=%s" % next_car_arrival_time
                # near_misses = {car_id: (Time, Distance)}
                near_misses[trial_car.get_viz_node_id()] = (
                    next_car_arrival_time, None)
        ...
        for t in sorted(participant_dict.iterkeys()):
            ...
            part_position = (participant.get_x_position(
                t), 0, participant.get_z_position(t))
            ...
            if (distance_to_car < 1 and is_near_missing == False):
                ...
                if near_missing_car_id not in near_misses.keys():
                    ...
                    near_misses[near_missing_car_id] = (
                        existing[0], closest_distance)
                is_near_missing = False
                closest_distance = 1
...
#			print "car=", k, near_misses[k]
    ...
    def calculateNearMisses(self, trialData):
        ...
        '''
        right_facing_cars_near_misses = self.__calculate_near_misses(
            trialData, Direction.RIGHT)
        left_facing_cars_near_misses = self.__calculate_near_misses(
            trialData, Direction.LEFT)
        # right_facing_cars_near_misses + left_facing_cars_near_misses}
        overall_near_misses = dict(
            right_facing_cars_near_misses.items() + left_facing_cars_near_misses.items())
    ...
    def calculateStartDelay(self, trialData):
        ...
        rightFacingCarsStartDelay = self.__calculate_start_delay(
            trialData, Direction.RIGHT)
        leftFacingCarsStartDelay = self.__calculate_start_delay(
            trialData, Direction.LEFT)
    ...
    def __calculate_start_delay(self, trialData, direction):
        ...
        if car_path_detail is None:
            print "S1"
            self.__logger.info(
                "StartDelay: Can't calculate start delay, participant never entered a car's path.")
            # return None
            return NO_VALUE_NUM

        enter_car_path_time = car_path_detail.get_enter_time()

        previous_car = trialData.get_previous_car_at_time(
            enter_car_path_time, direction)
        if previous_car is None:
            print "S2"
            self.__logger.info(
                "StartDelay: Can't calculate start delay, no car had passed when the child crossed.")
            # return None
            return NO_VALUE_NUM

        participant_x = trialData.get_participant().get_x_position(enter_car_path_time)
        start_delay = previous_car.get_time_from_x(
            enter_car_path_time, participant_x)
    ...
    def calculateTimeLeftToSpareOnEnterRoad(self, trialData):
        ...
        '''
        rightFacing = self.__calculate_time_left_to_spare_on_enter_road(
            trialData, Direction.RIGHT)
        leftFacing = self.__calculate_time_left_to_spare_on_enter_road(
            trialData, Direction.LEFT)
        return (rightFacing, leftFacing, None)

    def __calculate_time_left_to_spare_on_enter_road(self, trialData, direction):
        road_detail = trialData.get_last_enter_road_detail(
            ParticipantDirection.FORWARD)

        if road_detail is None:
            self.__logger.info(
                "TLS Enter Road: Can't calculate time left to spare on enter road, participant never entered the road.")
            # return None
            return NO_VALUE_NUM
        ...
        if trial_car is None:
            self.__logger.info(
                "No closest car.  Can't calculate TLS on enter road.")
            # return None
            return NO_VALUE_NUM
...
#		print "  TLS = %s" % tls
    ...
    def calculateTimeLeftToSpareOnEnterCarPath(self, trialData):
        ...
        rightFacing = self.__calculate_time_left_to_spare_on_enter_car_path(
            trialData, Direction.RIGHT)
        leftFacing = self.__calculate_time_left_to_spare_on_enter_car_path(
            trialData, Direction.LEFT)
        return (rightFacing, leftFacing, None)
    ...
    def __calculate_time_left_to_spare_on_enter_car_path(self, trialData, direction):
        ...
        if car_path_detail is None:
            ...
            self.__logger.info(
                "TLS Enter Car Path: Can't calculate, participant never entered a car's path")
            # return None
            return NO_VALUE_NUM
        ...
        if trial_car is None:
            self.__logger.info(
                "TLS Enter Car Path:, Can't calculate, no closest car.")
            # return None
            return NO_VALUE_NUM
...
#		print "  TLS = %s" % tls
    ...
    def calculateTimeLeftToSpareOnExitCarPath(self, trialData):
        ...
        rightFacing = self.__calculate_time_left_to_spare_on_exit_car_path(
            trialData, Direction.RIGHT)
        leftFacing = self.__calculate_time_left_to_spare_on_exit_car_path(
            trialData, Direction.LEFT)
        return (rightFacing, leftFacing, None)

    def __calculate_time_left_to_spare_on_exit_car_path(self, trialData, direction):
        car_path_detail = trialData.get_first_exit_car_path_detail(
            ParticipantDirection.FORWARD)
        if car_path_detail is None:
            self.__logger.info(
                "TLS Enter Car Path: Could not calculate, participant never exited the car path going forward.")
            # return None
            return NO_VALUE_NUM
#		else:
#			print "Exit CP at t=%s" % car_path_detail.get_exit_time()
    ...
    def _calculate_tls_on_exit_car_path_at_mean_walking_speed(self, trial_detail, mean_walking_speed):
        ...
        forward_cp_exits = [cp for cp in car_path_details if cp.get_exit_direction(
        ) == ParticipantDirection.FORWARD]
        if not forward_cp_exits:
            print "TLS@AVGSPEED: participant never exited a car path, returning NO_VALUE_NUM"
            return NO_VALUE_NUM
        last_exit_path = max(
            forward_cp_exits, key=lambda cp: cp.get_exit_time())
        ...
        closest_car = trial_detail.get_closest_car(
            time_entered_car_path, Direction.RIGHT)
        if not closest_car:
        ...
        position_exited_car_path = trial_detail.get_participant(
        ).get_position(last_exit_path.get_exit_time())
        distance_covered_in_car_path = vizmat.Distance(
            position_entered_car_path, position_exited_car_path)
        ...
        trial_times = sorted(
            trial_detail.get_participant().get_participant_dict().iterkeys())
        ...
        if estimated_exit_cp_t >= trial_times[-1]:
            ...
            car_dist_on_enter = -closest_car.get_front_bumper_distance_from_x(
                time_entered_car_path, position_exited_car_path[Position.X_POS])
            car_dist_on_exit_at_avg = car_dist_on_enter - \
                (car_speed * time_to_cross_at_average)
        ...
        # otherwise we get a matching trial timestamp, find where the car was at that time, and find tls that way
            ...
            t_index = min(range(len(trial_times)), key=lambda i: abs(
                trial_times[i] - estimated_exit_cp_t))
            t = trial_times[t_index]
            car_speed = closest_car.get_velocity_x(t)
            car_dist = -closest_car.get_front_bumper_distance_from_x(
                t, position_exited_car_path[Position.X_POS])
    ...
    def calculate_time_left_to_spare_on_loss_of_view(self, trial_detail):
        ...
        next_car_to_participant = trial_detail.get_closest_car(
            t_last_entered_road, Direction.RIGHT)
        ...
        tls_on_loss_of_view = next_car_to_participant.get_time_from_x(
            t, last_entered_road.get_x_position())
    ...
    def calculate_head_angle_on_loss_of_view(self, trial_data):
        ...
        if not last_entered_road:
            return NO_VALUE_NUM
        t_last_entered_road = last_entered_road.get_time()
        next_car_to_participant = trial_data.get_closest_car(
            t_last_entered_road, Direction.RIGHT)
        ...
        head_angle = participant.get_orientation_data(
        )[prev_moment.get_index()][OrientationData.ORIENTATION][OrientationType.YAW]
        return head_angle
    ...
    def _is_car_visible(self, closest_car, participant, t):
        ...
        # TODO: this will need to be updated if a different headset is used
        participant_fov = Globals.headset_fov
        participant_position = participant.get_position(t)

        # get a triangle ABC representing the participant's vision in the x-z plane
        point_a = (participant_position[Position.X_POS],
                   participant_position[Position.Z_POS])
        point_b, point_c = self._solve_vision_triangle(
            participant_yaw, participant_fov, participant_position)

        # get the corners of the car
        car_corner_points = [(closest_car.get_front_bumper_x(t), closest_car.get_far_side_z(t)),  # front far side
                             (closest_car.get_front_bumper_x(
                                 t), closest_car.get_close_side_z(t)),  # front close side
                             (closest_car.get_rear_bumper_x(
                                 t), closest_car.get_close_side_z(t)),  # rear close side
    ...
    def _solve_vision_triangle(self, participant_yaw, participant_fov, participant_position):
        ...
        # max distance (m) they can see (easier to solve as triangle than infinite 2D cone)
        view_distance = 350.0
        participant_fov = math.radians(participant_fov)
        # trust me on this one
        participant_yaw = -math.radians(participant_yaw)
        x = participant_position[Position.X_POS]
        z = participant_position[Position.Z_POS]
        r = view_distance / math.cos(participant_fov / 2.0)

        point_b = (x - (r * math.sin(participant_yaw - (participant_fov / 2.0))),
                   z + (r * math.cos(participant_yaw - (participant_fov / 2.0))))
        point_c = (x - (r * math.sin(participant_yaw + (participant_fov / 2.0))),
                   z + (r * math.cos(participant_yaw + (participant_fov / 2.0))))
    ...
    def calculateGapLength(self, trialData):
        ...
        rightGapLength = self.__calculate_gap_length(
            trialData, Direction.RIGHT)
    ...
    def __calculate_gap_length(self, trialData, direction):
        ...
        if car_path_detail is None:
            self.__logger.info(
                "Gap Length: can't calculate, participant never enter a car's path.")
            # return None
            return NO_VALUE_NUM
        ...
        previous_car = trial_cars.get_previous_car_passed_x_at_t(
            participant_x, enter_time)
        next_car = trial_cars.get_next_to_x_at_t(participant_x, enter_time)

        if previous_car is None or next_car is None:
            self.__logger.info(
                "Gap Length: Can't calculate, no cars passed or there's no next car.")
            if direction == Direction.RIGHT:
    ...
    def calculateGapInterval(self, trialData):
        ...
        rightGapInterval = self.__calculate_gap_interval(
            trialData, Direction.RIGHT)
        leftGapInterval = self.__calculate_gap_interval(
            trialData, Direction.LEFT)
    ...
    def __calculate_gap_interval(self, trialData, direction):
        ...
        if car_path_detail is None:
            self.__logger.info(
                "GapInterval: Can't calculate gap interval, participant never entered into a car's path.")
...
#		print "ENTER TIME = %s" % enter_time
        ...
        if next_car is None:
            self.__logger.info(
                "Gap Interval: Cannot calculate gap interval, no next car.")
            return None
        ...
        if gap_length is None or gap_length is NO_VALUE_NUM:
            self.__logger.info(
                "Gap Interval: Cannot calculate gap interval, no cars passed.")
            if direction == Direction.RIGHT:
    ...
    def calculatePercentageOfGapUsed(self, trialData):
        ...
        rightPercentage = self.__calculate_percentage_of_gap_used(
            trialData, Direction.RIGHT)
        leftPercentage = self.__calculate_percentage_of_gap_used(
            trialData, Direction.LEFT)
    ...
    def calculatePercentageOfGapRemainingOnEnterCarPath(self, trialData):
        ...
        rightPercentage = self.__calculate_percentage_of_gap_remaining_on_enter_car_path(
            trialData, Direction.RIGHT)
        leftPercentage = self.__calculate_percentage_of_gap_remaining_on_enter_car_path(
            trialData, Direction.LEFT)
        overallPercentage = None
        return (rightPercentage, leftPercentage, overallPercentage)
    ...
    def calculatePercentageOfGapRemainingOnExitCarPath(self, trialData):
        ...
        rightPercentage = self.__calculate_percentage_of_gap_remaining_on_exit_car_path(
            trialData, Direction.RIGHT)
        leftPercentage = self.__calculate_percentage_of_gap_remaining_on_exit_car_path(
            trialData, Direction.LEFT)
        overallPercentage = None
        return (rightPercentage, leftPercentage, overallPercentage)
    ...
    def calculateMarginOfSafety(self, trialData):
        ...
        '''
        rightPercentage = self.__calculate_margin_of_safety(
            trialData, Direction.RIGHT)
        # None #self.__calculate_margin_of_safety(trialData, Direction.LEFT)
        leftPercentage = NO_VALUE_NUM
        overallPercentage = NO_VALUE_NUM  # None
        return (rightPercentage, leftPercentage, overallPercentage)

    def __calculate_margin_of_safety(self, trialData, direction):
        margin_of_safety = None
        tls_enter_car_path = self.__calculate_time_left_to_spare_on_enter_car_path(
            trialData, direction)
        tls_exit_car_path = self.__calculate_time_left_to_spare_on_exit_car_path(
            trialData, direction)
        if tls_enter_car_path is None or tls_exit_car_path is None:
            ...
            self.__logger.info(
                "Margin of Safety: Could not calculate, participant didn't ever exit a car's path.")
            # return None
            return NO_VALUE_NUM
...
#		print "tls_exit_car_path = %s" % tls_exit_car_path
        ...
        margin_of_safety = (
            (tls_enter_car_path / (tls_enter_car_path - tls_exit_car_path)) - 1) * 100
    ...
    def __calculate_percentage_of_gap_used(self, trialData, direction):
        ...
        if car_path_detail is None:
            ...
            self.__logger.info(
                "% Gap Used: Could not calculate, participant didn't ever enter a car's gap.")
            # return None
            return NO_VALUE_NUM
...
#		print "gap interval = %s" % gap_interval
    ...
    def calculateCrossingTime(self, trialData):
        ...
        road_detail = trialData.get_last_exit_road_detail(
            ParticipantDirection.FORWARD)
        if road_detail is None:
            self.__logger.info(
                "Crossing Time: can't calculate, participant never crossed the road.")
            # return None
            return NO_VALUE_NUM
    ...
    def calculateAverageRejectedGapTime(self, trialData):
        ...
        rightFacingAverageRejectedGapTime = self.__calculate_average_rejected_gap_time(
            trialData, Direction.RIGHT)
        leftFacingAverageRejectedGapTime = self.__calculate_average_rejected_gap_time(
            trialData, Direction.LEFT)
...
#		print "gaps = %s" % [str(x) for x in gaps]
        ...
        avg_rejected_gap_time = sum(gap.get_gap_time()
                                    for gap in gaps[:-1]) / (len(gaps) - 1)
    ...
    def calculateMaxRejectedGapTime(self, trialData):
        ...
        rightFacingMaxRejectedGapTime = self.__calculate_max_rejected_gap_time(
            trialData, Direction.RIGHT)
        leftFacingMaxRejectedGapTime = self.__calculate_max_rejected_gap_time(
            trialData, Direction.LEFT)
    ...
    def __calculate_max_rejected_gap_time(self, trialData, direction):
        ...
        if len(gaps) <= 1:
            ...
            self.__logger.info(
                "Maximum Rejected Gap Time: Could not calculate max rejected gap time.")
            # return None
            return NO_VALUE_NUM
    ...
    def calculateMinRejectedGapTime(self, trialData):
        ...
        rightFacingMinRejectedGapTime = self.__calculate_min_rejected_gap_time(
            trialData, Direction.RIGHT)
        leftFacingMinRejectedGapTime = self.__calculate_min_rejected_gap_time(
            trialData, Direction.LEFT)
    ...
    def __calculate_min_rejected_gap_time(self, trialData, direction):
        ...
        if len(gaps) <= 1:
            self.__logger.info(
                "Minimum Rejected Gap Time: Could not calculate min rejected gap time.")
            return NO_VALUE_NUM
    ...
    def calculateMissedOpportunities(self, trialData):
        ...
        rightFacingMissedOpportunities = self.__calculate_missed_opportunities(
            trialData, Direction.RIGHT)
        leftFacingMissedOpportunities = self.__calculate_missed_opportunities(
            trialData, Direction.LEFT)
    ...
    def __calculate_missed_opportunities(self, trialData, direction):
        ...
        if len(gaps) <= 0:
            ...
            self.__logger.info(
                "Missed Opportunities: can't calculate, there were no gaps in the trial")
            # return None
            return NO_VALUE_NUM
        ...
        if road_detail is None:
            self.__logger.info(
                "Missed Opportunities: can't calculate, participant did not cross the road safely.")
            # return None
            return NO_VALUE_NUM
        ...
        missed_opportunities = filter(lambda gap_time: gap_time > (
            1.5 * crossing_time), [gap.get_gap_time() for gap in gaps[:-1]])  # last gap is the crossing/hit gap
    ...
    def getNearMissDistance(self, near_miss_dict):
        ...
        it = [k for k in near_miss_dict.keys() if near_miss_dict.get(k)[
            NEAR_MISS_DISTANCE] is not None]
        if it:
            k = min(it, key=lambda x: near_miss_dict.get(
                x)[NEAR_MISS_DISTANCE])
    ...
    def getNearMissTime(self, near_miss_dict):
        ...
        it = [k for k in near_miss_dict.keys() if near_miss_dict.get(k)[
            NEAR_MISS_TIME] is not None]
        if it:
            k = min([k for k in near_miss_dict if near_miss_dict[k][NEAR_MISS_TIME]
                     is not None], key=lambda x: near_miss_dict.get(x)[NEAR_MISS_TIME])
    ...
    def calculateTrafficVolume(self, trialData):
        ...
        rightFacingTrafficVolume = self.__calculate_traffic_volume(
            trialData, Direction.RIGHT)
        leftFacingTrafficVolume = self.__calculate_traffic_volume(
            trialData, Direction.LEFT)
    ...
    def calculateTrafficVolumePerMinute(self, trialData):
        ...
        rightFacingTrafficVolumePerMinute = self.__calculate_traffic_volume_per_minute(
            trialData, Direction.RIGHT)
        leftFacingTrafficVolumePerMinute = self.__calculate_traffic_volume_per_minute(
            trialData, Direction.LEFT)
        overallTrafficVolumePerMinute = rightFacingTrafficVolumePerMinute + \
            leftFacingTrafficVolumePerMinute
    ...
    def __calculate_traffic_volume_per_minute(self, trialData, direction):
        ...
        if(last_time == None):
            ...
            self.__logger.error(
                "ERROR: in calculate_traffic_volume_per_minute, first_car_time and last_time are the same. Setting to 99999")
            return NO_VALUE_NUM
        cars_passed = self.__calculate_traffic_volume(trialData, direction)
        traffic_vol_per_minute = cars_passed / \
            ((last_time - first_car_time) / 60)
